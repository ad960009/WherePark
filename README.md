...
*   **자동 주차 기록:** 차량 블루투스(페어링된 기기) 연결 및 해제를 감지하면 자동으로 포그라운드 서비스를 실행하여 주행 상태를 유지하거나 5분간 주변 비컨을 스캔합니다.
*   **리소스 최적화:** 평상시에는 백그라운드 리시버로 대기하다가 필요할 때만 포그라운드 서비스를 가동하며, 탐색이 완료되면 서비스를 자동으로 종료하여 배터리 소모를 최소화합니다.
